...
def generate_distance_matrix():
    ...
    for i in range(cities_count):
        for j in range(cities_count):
            distance_matrix[i][j] = round(((cities[i].x - cities[j].x) ** 2 + (cities[i].y - cities[j].y) ** 2) ** 0.5,
                                          2)
...
def start():
    ...
    if is_already_started:
        messagebox.showwarning(title='Внимание',
                               message='Процесс уже запущен. Вы можете преждевременно прервать его, нажав СБРОС')
